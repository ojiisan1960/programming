...
MNT_DIR_ROOT="/mnt/$HOSTNAME"
RSYNC="rsync -e ssh --delete -avx"

# system image definitions
ROOT_DISK_IMAGE="$HOSTNAME.img"
MOUNT_ROOT_IMAGE="lomount -diskimage $IMAGE_DIR/$ROOT_DISK_IMAGE -partition 1 $MNT_DIR_ROOT"
#ROOT_DIRS="/bin /boot /etc /export /home /lib /root /sbin /usr /dev"
ROOT_DIRS="/bin /boot /etc /export /home /lib /root /sbin /usr"
...
echo "Update root directory stucture..."
echo rsync -vd / $SYNC_HOST:$MNT_DIR_ROOT/
rsync -vld / $SYNC_HOST:$MNT_DIR_ROOT/
echo $RSYNC $ROOT_DIRS $SYNC_HOST:$MNT_DIR_ROOT/
$RSYNC $ROOT_DIRS $SYNC_HOST:$MNT_DIR_ROOT/
    if [ $? -gt 0 ];then
